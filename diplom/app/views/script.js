...
function openDialogReg() {
    $.ajax({
        url: window.BASE_DIR_AJAX + "/regForm.php",
        dataType: "html",
        success: function (html) {
            $('body').append(html);
            let modalObj = new bootstrap.Modal(document.getElementById('reg_form'));
            $('#reg_form').on('hide.bs.modal', function (event) {
                $(this).remove();
            })
...
function globalReg(event) {
    event.preventDefault();
    let form = $(event.target);
    let form_data = form.serializeArray();
    $.ajax({
        url: "/kryuchkova/diplom/reg/registration/",
        type: "POST",
        data: form_data,
        dataType: "json",
        beforeSend: function () {
